...
## Purpose
Bounded worker execution, atomic semaphore concurrency control, async node retries, and checkpoint pruning in the orchestrator.
...
### Requirement: Orchestrator graceful shutdown
The orchestrator lifespan SHALL set `app.state.is_shutting_down = True` on shutdown, reject new `/run` requests with HTTP 503, and drain `graph_semaphore` slots before closing DB/HTTP connections.

#### Scenario: Orchestrator receiving SIGTERM
- **WHEN** the orchestrator process receives a shutdown signal
- **THEN** it rejects new requests with 503 and waits up to 5s per semaphore slot for in-flight tasks to complete before closing connections
