# pre-production-qa Specification
...
The test suite SHALL provide a load generator in `scripts/benchmark.py` that simulates 20+ concurrent user sessions against the Gateway and outputs P95 and P99 latency distribution.

#### Scenario: High concurrency load test
- **WHEN** 20 concurrent workers send requests to `http://localhost:8000/v1/run`
