# shared-http-client-factory Specification

## Purpose
Factory functions and helpers for HTTP client creation across all services.

## Requirements
### Requirement: Async HTTP client factory accepts structured timeout
The `create_async_http_client()` function in `shared/http_client.py` SHALL accept an optional `httpx.Timeout` object as a parameter, using a sensible default when omitted (`connect=5.0, read=120.0, write=10.0, pool=5.0`). All services that need a persistent async HTTP client SHALL use this factory. No service SHALL construct `httpx.AsyncClient` directly with inline timeout kwargs.
...
#### Scenario: Action service uses factory
- **WHEN** the action service lifespan initialises its persistent HTTP client
- **THEN** it calls `create_async_http_client()` from `shared.http_client` (not `httpx.AsyncClient(timeout=httpx.Timeout(...))` directly)

#### Scenario: Orchestrator inline client replaced
- **WHEN** the orchestrator calls `_fetch_session_messages()`
- **THEN** it uses a shared client stored on `app.state` (not an inline `async with httpx.AsyncClient(...)`), ensuring the connection is reused and properly closed at shutdown
