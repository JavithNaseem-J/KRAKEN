...
The repository SHALL NOT contain live API keys, tokens, passwords, or other credentials in any tracked file, including `.env`. Tracked environment template files (e.g., `.env.example`) SHALL contain only placeholders. Real secrets SHALL be sourced exclusively from environment variables injected by the deployment platform or a secrets manager. Any credential that has been committed SHALL be rotated at the provider before this change is considered complete.
