...
### Requirement: Gateway validates request payload structure
The Gateway `/v1/run` endpoint SHALL parse and validate incoming JSON bodies against `QueryRequest` prior to `PromptGuardMiddleware` or proxy dispatch.

#### Scenario: Invalid JSON or model failure
- **WHEN** a client sends invalid JSON or payload failing `QueryRequest` validation
- **THEN** Gateway returns HTTP 422 Unprocessable Entity
