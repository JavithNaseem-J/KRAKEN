...
def seed_postgres() -> None:
    ...
    try:
        ...
        with ConnectionPool(conninfo=pg_url, timeout=5) as pool, pool.connection() as conn:
            if SYNTHETIC_TICKETS_FILE.exists():
                tickets = json.loads(SYNTHETIC_TICKETS_FILE.read_text(encoding="utf-8"))
...
def main() -> None:
    if not SYNTHETIC_TICKETS_FILE.exists():
        print(f"Error: Canonical tickets file not found at '{SYNTHETIC_TICKETS_FILE}'.")
        sys.exit(1)

    WORKSPACE_TICKETS_FILE.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(SYNTHETIC_TICKETS_FILE, WORKSPACE_TICKETS_FILE)
    print(
        f"Successfully copied ticket database: '{SYNTHETIC_TICKETS_FILE}' -> '{WORKSPACE_TICKETS_FILE}'"
    )
