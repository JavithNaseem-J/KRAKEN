...
2. ACTION SELECTION CRITERIA:
   ...
   - Use 'quarantine_ip' whenever the user asks to block, ban, isolate, or quarantine an IP address (e.g. suspicious port scan, malicious traffic). Do NOT default to 'auto_respond' to give advice; select 'quarantine_ip' so the operator can review and approve the block. Extract and populate action_payload with 'ip', 'reason', and 'evidence'.
